...
gameStationInfo = {'武汉站': ['a', 0, 0, 0], '动车所': ['b', 0, 0, 20],
                   '汉口': ['e', 2, 1, 5],
                   '孝感北': ['d', 1, 2, 4], '红安西': ['k', 1, 2, 4],
                   '咸宁北': ['c', 2, 1, 3],
                   '鄂州': ['f', 2, 1, 5], '黄冈': ['g', 2, 1, 5],
                   '葛店南站': ['葛店南', 0, 0, 5],
                   }
gameStationDefault = ['未知',  0, 0, 20]  # 找不到的默认设置
...
def routeStrFormate(prepareList: list[list]):
    ...
    traingpdf["arrival"][0][0] = traingpdf["arrival"][0][1] - \
        datetime.timedelta(
            minutes=(gameStationInfo.get(traingpdf["station"][0][0], gameStationDefault))[3])
    traingpdf["departure"][0][2] = traingpdf["departure"][0][1] + \
        datetime.timedelta(
            minutes=(gameStationInfo.get(traingpdf["station"][0][2], gameStationDefault))[3])
...
def generateGameStr(infodf: pandas.DataFrame, entRule=[]):
    ...
    arrStr = "{sta}#{rail}#{clck}#{stay} ".format(
        sta=astref[0], rail=astref[1], clck=infodf["arrival"][0][0], stay=0)
    dst = infodf["station"][0][2]  # 离场部分
    dstref = gameStationInfo.get(dst, gameStationDefault)
    depStr = "{sta}#{rail}#{clck}#{stay} ".format(
        sta=dstref[0], rail=dstref[2], clck=infodf["departure"][0][2], stay=0)
